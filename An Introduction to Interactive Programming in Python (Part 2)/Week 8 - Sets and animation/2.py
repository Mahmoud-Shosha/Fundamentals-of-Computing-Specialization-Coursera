"""
Our next task will be to prevent a new bubble from being created until the previous one is 'stuck'.
To this end, create a global Boolean flag called bubble_stuck and initialize it to be True.
When the space bar is pressed and a bubble is created, set bubble_stuck to be False and ignore all presses of
the space bar until bubble_stuck is True. Add code to the draw handler that checks whether the bubble 'is stuck'
(i.e., the previous exercise's method) to something, such as the top wall, and then sets bubble_stuck to be True
as well as resetting a_bubble to a new bubble waiting to be fired. In this step, the bubble should reset at the firing
position when it strikes the top wall.
"""
