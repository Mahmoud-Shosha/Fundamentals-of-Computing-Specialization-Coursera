"""
Finally, modify the is_stuck method to take a set of bubbles as a parameter and use the collide method to test if
the bubbles has collided with the top wall or any of the bubbles in the set.
The result of this modification should a program that has the same basic physics as Bubble Shooter where bubbles stick
to other bubbles when they collide. Feel free to take it from here and experiment!
"""
...
class Bubble:
    ...
    def is_stuck(self, bubbles_set):
        stuck = False
        for bubble in bubbles_set:
            if self.collide(bubble):
                stuck = True
        if self.pos[1] <= BUBBLE_RADIUS or stuck:
            return True
        return False
