...
def draw(canvas):
    ...
    space_collision = group_collide(rock_group, my_ship)
    if space_collision:
        lives -= 1
    if lives == 0:
        started = False
        ...
        missile_group = set([])
        soundtrack.rewind()
    rocks_missiles_collisions = group_group_collide(rock_group, missile_group)
    score += rocks_missiles_collisions
...
def rock_spawner():
    if started and len(rock_group) < 12:
        rock_vel_increment = score // 8
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * .6 - .3 + rock_vel_increment,
                    random.random() * .6 - .3 + rock_vel_increment]
        rock_avel = random.random() * .2 - .1
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        distance = dist(a_rock.pos, my_ship.pos)
        radius_sum = a_rock.radius + my_ship.radius
        too_close = distance <= radius_sum + 10
        if not too_close:
...
def group_collide(group, other_object):
    ...
    for sprite in list(group):
        if sprite.collide(other_object):
            a_collision = Sprite(sprite.pos, [0, 0], sprite.angle, 0,
